...
//$db->debug =true;
$hbook_config = get_config('hiperbook');
...
$navigationnum     = optional_param('navigationnum', 0, PARAM_INT);     // se exibe ou nao a navegação 
...
// busca a navegacao atual
	// se professor ou grupomode = visiveis ou sem grupos, mostra todos os caminhos
	if(( $isteacher) || ($cm->groupmode!=SEPARATEGROUPS)) {
	
		//$navpath = get_record_sql('select * from '.$CFG->prefix.'hiperbook_navigationpath np where  np.bookid =  '.$book->id .' and np.navpathnum=1' );	
		
		
		
	}else{
		// mostra o navpath apenas do grupo selecionado ou todos os participantes
	//	$navpath = get_record_sql('select * from '.$CFG->prefix.'hiperbook_navigationpath  where id  = '.$navchapter->navigationid.' and (groupid = '.$groupid.' or groupid = 0)');			
	}	
...
if (($pop_up==1)||( substr_count($_SERVER['HTTP_REFERER'],'popup.php') >= 1)) { 
	// redirect($CFG->wwwroot."/mod/hiperbook/popup.php?id=".$id."&chapterid=".$chapterid."&edit=".$edit."&pagenum=".$pagenum."&navigationnum=".$navigationnum."&parentnavchapterid=".$parentnavchapterid);
}
...
$totalpages = count_records('hiperbook_chapters_pages', 'chapterid', $chapter->id);
	...
	// security checks  END
	// =========================================================================
	add_to_log($course->id, 'hiperbook', 'view', 'view.php?id='.$cm->id.'&target_navigation_chapter='.$navchapter->id, $book->id, $cm->id);
	...
	$buttons = $isteacher ? '<table cellspacing="0" cellpadding="0"><tr><td>'.update_module_button($cm->id, $course->id, $strbook).'</td>'.
			   '<td>&nbsp;</td><td>'.hiperbook_edit_button($cm->id, $course->id, $navchapter->chapterid,$navigationnum, $pagenum,  $target_navigation_chapter).'</td></tr></table>'
			   : '&nbsp;';
...
?>
	...
	if ($page){	
	   if ($totalpages > $pagenum) {
		...
		{ $pgnavigation .= '<a title="adicionar anterior" href="page.php?id='.$cm->id.'&target_navigation_chapter='.$navchapter->id.'&pagenum='.$page->pagenum.'&addprev=1&groupid='.$groupid.'"><img src="pix/add.gif"></a> '; }		
		
		
		
		if ($prevpage!='') {
			$pgnavigation .= '<a  href="view.php?id='.$cm->id.'&pagenum='.$prevpage .'&target_navigation_chapter='.$target_navigation_chapter.'&show_navigation='.$show_navigation.'"><img src="'.$img_page_prev.'"/></a> ';
		} 	
  ...
  <input type="hidden" name="show_navigation" id="show_navigation" value="'.$show_navigation.'">';
		...
		if ($nextpage !='') {
			$pgnavigation .= '<a " href="view.php?id='.$cm->id.'&pagenum='.$nextpage .'&target_navigation_chapter='.$target_navigation_chapter.'&show_navigation='.$show_navigation.'"><img src="'.$img_page_next.'"/></a>';
		}	
...
//	var_dump($groupmode);
	...
	if($navchapter->id !=0){
		$breadcrumbs .= '<td id="current"><div id="start">&nbsp;</div><div id="main"><a href="view.php?id='.$cm->id.'&pagenum=1&navigationnum='.$navpath->navpathnum.'&target_navigation_chapter=0&show_navigation='.$show_navigation.'">'. $navpath->name.'</a></div><div id="end">&nbsp; </div></td>'. hiperbook_link_parent_chapters($navchapter->id, $cm, $groupid);	
}
...
<input type="hidden" name="chapterid" value="<?php echo $navchapter->chapterid?>" />
...
if(count($navpaths) > 1){	
	foreach($navpaths as $np){		
		...
		$nav_chapter_aux = get_record('hiperbook_navigation_chapters', 'parentnavchapterid','0', 'navigationid',$np->id);
			?>
			<td id="past" ><? if ($np->navpathnum=='1'){ ?> <div id="start">&nbsp;</div><? } ?><div id="main"><a href="view.php?id=<?php echo $cm->id.'&navigationnum='.$np->navpathnum.'&groupid='.$groupid ?>"> <?= $np->name ?> </a></div><? if ($np->navpathnum==count($navpaths)){ ?><div id="end">&nbsp;</div><? } ?></td><? 
		} 
	}
}
...
//$glossary = get_records_sql(" select h.title, h.content from ". $CFG->prefix."hiperbook_chapters c,". $CFG->prefix."hiperbook_chapters_hotword h where c.bookid = '$book->id' and c.id = h.chapterid",true);
	...
	if (($edit)||($page->opentostudents)){ // para adicionar?>
	 <a href="javascript:void(0);"  title='Links' onclick="javascript:window.open('links.php?chapterid=<?= $navchapter->chapterid ?>&edit=1&id=<?=  $cm->id ?>&pageid=<?php echo $page->id ?>','links','menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=650,height=550,left=80,top=80')" ><img src="<?php echo $img_links_icon ?>" name="Image2"  border="0" /></a> 
	 <a href="javascript:void(0);" title='Glossário' onClick="javascript:window.open('hotword.php?chapterid=<?= $navchapter->chapterid ?>&pageid=<?= $pageid ?>&edit=1&id=<?=  $cm->id ?>&groupid=<?php echo $groupid; ?>','hotword','menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=650,height=550,left=80,top=80')" ><img src="<?php echo $img_hotwords_icon ?>"  name="Image1" border="0" /></a> 
	 <a  href="javascript:void(0);"  title='Dicas'  onClick="javascript:window.open('tips.php?chapterid=<?= $navchapter->chapterid ?>&pageid=<?= $pageid ?>&edit=1&id=<?=  $cm->id ?>&groupid=<?php echo $groupid?>','dicas','menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=650,height=550,left=80,top=80');"  ><img src="<?php echo $img_tips_icon ?>" name="Image4"  border="0" /></a>  
	 <a href="javascript:void(0);"  title='Bibliografia' onClick="javascript:window.open('suggestions.php?chapterid=<?= $navchapter->chapterid ?>&pageid=<?= $pageid ?>&edit=1&id=<?=  $cm->id ?>&groupid=<?php echo $groupid?>','dicas','menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=650,height=550,left=80,top=80')"   ><img src="<?php echo $img_suggestions_icon ?>" name="Image3" border="0" /></a> 
		 <? 
	}else{ 
	 // soh mostra glossario e bibliografia (suggestions)
	 		//if($glossary){
		   if(true){	?>
		    <a href="javascript:void(0);"  title='Glossário' href="javascript:void(0);" onClick="javascript:window.open('glossary.php?chapterid=<?= $navchapter->chapterid ?>&edit=<?= $edit ?>&id=<?=  $cm->id ?>&groupid=<?php echo $groupid?>','dicas','menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=650,height=550,left=80,top=80')" ><img src="<?php echo $img_hotwords_icon ?>"  border="0" /></a>
		 <? }
		  if($suggestion){	?> 
		 <a href="javascript:void(0);"  title='Bibliografia' onClick="javascript:window.open('suggestions.php?chapterid=<?= $navchapter->chapterid ?>&pageid=<?= $pageid ?>&edit=0&id=<?=  $cm->id ?>&groupid=<?php echo $groupid?>','dicas','menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=650,height=550,left=80,top=80')"><img src="<?php echo $img_suggestions_icon ?>"  border="0" /></a>
		  <? } 		  
	} // edit
} // page ?></div>
