...
/// included from mod/book/popup.php and print.php
///
/// uses:
...
if (($edit)||($navpath->opentostudents)) { ///teacher's TOC
	...
	//	if ($ch->parentnavchapterid == $navchapterid){
		...
		//	echo 'PARENT'.$parentchapternums;
			...
			if($ch->groupid == $groupid ||$ch->groupid == 0 ) {
				$toc .= '<span id="item">'.$parentchapternums .' <a title="'.htmlspecialchars($title).'" href="popup.php?id='.$cm->id.'&groupid='.$groupid.'&target_navigation_chapter='. $ch->target_navigation_chapter.'">'.$title.'</a>';	 
				$toc .=  '&nbsp;&nbsp;</span>';
			}else{
	...
	//	if ($chp->parentnavchapterid == $navchapterid){
			...
			if($chp->groupid == $groupid ||$chp->groupid == 0 ) {
				$toc .= '<span id="item">'.$parentchapternums .'<li> <a title="'.htmlspecialchars($title).'" href="popup.php?id='.$cm->id.'&groupid='.$groupid.'&target_navigation_chapter='. $chp->target_navigation_chapter.'">'.$title.'</a></li>';	 
				$toc .=  '&nbsp;&nbsp;</span>';
			}else{
            ...
            if ($ch->id === $chapter->id) {
                ...
                $toc .= '<a title="'.htmlspecialchars($title).'" href="popup.php?id='.$cm->id.'&chapterid='.$ch->id.'">'.$title.'</a>';
            }
